...
namespace TvDatabase
{
  ...
  public class Schedule : Persistent
  {
    ...
    [TableColumn("seriesStartRangeOffset", NotNull = true)] private int seriesStartRangeOffset;
    ...
    public Schedule(int idChannel, int idParentSchedule, int scheduleType, string programName, DateTime startTime,
                    ...
                    int preRecordInterval, int postRecordInterval, DateTime canceled, int seriesStartRangeOffset = 0)
    {
      isChanged = true;
      this.idChannel = idChannel;
      ...
      recommendedCard = -1;
      this.seriesStartRangeOffset = seriesStartRangeOffset;
    }
    ...
    public Schedule(Schedule schedule)
    {
      ...
      recommendedCard = -1;
      seriesStartRangeOffset = schedule.seriesStartRangeOffset;
    }
    ...
    public Schedule(int idSchedule, int idParentSchedule, int idChannel, int scheduleType, string programName,
                    ...
                    DateTime keepDate, int preRecordInterval, int postRecordInterval, DateTime canceled, int seriesStartRangeOffset = 0)
    {
    ...
    /// <summary>
    /// Property to configure the time in minutes before and after a schedule's StartTime that
    /// a program should be recorded when part of a Series recording.
    /// 
    /// A value of 0 indicates there is no start range defined and all programs should be recorded
    /// regardless of program StartTime.
    /// </summary>
    public int SeriesStartRangeOffset
    {
      get { return seriesStartRangeOffset;  }
      set
      {
        isChanged |= seriesStartRangeOffset != value;
        seriesStartRangeOffset = value;
      }
    }
    ...
    public static IList<Program> GetProgramsForSchedule(Schedule schedule)
    {
      ...
      switch (schedule.scheduleType)
      {
        ...
        case (int)ScheduleRecordingType.EveryTimeOnEveryChannel:
          progs = schedule.ApplySeriesStartRangeFilter(Program.RetrieveEveryTimeOnEveryChannel(schedule.programName));
          break;

        case (int)ScheduleRecordingType.EveryTimeOnThisChannel:
          progs = schedule.ApplySeriesStartRangeFilter(Program.RetrieveEveryTimeOnThisChannel(schedule.programName, schedule.ReferencedChannel().IdChannel));
          break;

        case (int)ScheduleRecordingType.WeeklyEveryTimeOnThisChannel:
          progs = schedule.ApplySeriesStartRangeFilter(Program.RetrieveWeeklyEveryTimeOnThisChannel(schedule.startTime, schedule.endTime, schedule.programName, schedule.ReferencedChannel().IdChannel));
          break;
    ...
    public IList<Program> ApplySeriesStartRangeFilter(IList<Program> programs)
    {
      if (programs == null || !IsSeriesStartRangeEnabled())
        return programs;

      return programs.Where(p => this.IsInsideSeriesStartRange(p)).ToList();
    }

    public bool IsSeriesStartRangeEnabled()
    {
      switch ((ScheduleRecordingType)this.ScheduleType)
      {
        case ScheduleRecordingType.EveryTimeOnThisChannel:
        case ScheduleRecordingType.EveryTimeOnEveryChannel:
        case ScheduleRecordingType.WeeklyEveryTimeOnThisChannel:

          return SeriesStartRangeOffset > 0;
    ...
    public bool IsInsideSeriesStartRange(DateTime currentTime)
    {
      if (!IsSeriesStartRangeEnabled())
        return true;

      var startRange = this.startTime.AddMinutes(-SeriesStartRangeOffset);
      var endRange = this.startTime.AddMinutes(SeriesStartRangeOffset);

      var currentTimeWithStartRangeDate = new DateTime(startRange.Year, startRange.Month, startRange.Day, currentTime.Hour, currentTime.Minute, currentTime.Second);

      if (startRange <= currentTimeWithStartRangeDate && currentTimeWithStartRangeDate <= endRange)
        return true;

      var currentTimeWithEndRangeDate = new DateTime(endRange.Year, endRange.Month, endRange.Day, currentTime.Hour, currentTime.Minute, currentTime.Second);

      if (startRange <= currentTimeWithEndRangeDate && currentTimeWithEndRangeDate <= endRange)
        return true;

      return false;
    }

    public bool IsInsideSeriesStartRange(Program program)
    {
      if (program == null)
        return false;

      return IsInsideSeriesStartRange(program.StartTime);
    }
    ...
    public bool IsRecordingProgram(Program program, bool filterCanceledRecordings)
    {
      ...
      switch (scheduleRecordingType)
      {
        case ScheduleRecordingType.Once:
          ...
          break;
        case ScheduleRecordingType.EveryTimeOnEveryChannel:
          if (program.Title == ProgramName && IsInsideSeriesStartRange(program))
          {
          ...
          break;
        case ScheduleRecordingType.EveryTimeOnThisChannel:
          if (program.Title == ProgramName && program.IdChannel == IdChannel && IsInsideSeriesStartRange(program))
          {
            if (filterCanceledRecordings && IsSerieIsCanceled(GetSchedStartTimeForProg(program), program.IdChannel))
            {
              return false;
        ...
        case ScheduleRecordingType.WeeklyEveryTimeOnThisChannel:
          if (program.Title == ProgramName && program.IdChannel == IdChannel &&
              StartTime.DayOfWeek == program.StartTime.DayOfWeek && IsInsideSeriesStartRange(program))
          {
            if (filterCanceledRecordings && IsSerieIsCanceled(GetSchedStartTimeForProg(program), program.IdChannel))
            {
              return false;
    ...
    public Schedule Clone()
    {
      Schedule schedule = new Schedule(IdChannel, idParentSchedule, scheduleType, ProgramName, StartTime, EndTime,
                                       ...
                                       Canceled, SeriesStartRangeOffset);
